...
Correct Port Mapping: Ensure the domain is using the correct port for your application. For example, if you're using Next.js, the port should be`3000`, or for Laravel, it should be`8000`. If you change the app port, update the domain to reflect that.

Avoid Using`Ports` in Advanced Settings: Generally, there's no need to use the`Ports` feature unless you want to access your app via`IP:port`. Leaving this feature enabled may interfere with your domain.
...
Listen on 0.0.0.0, Not 127.0.0.1: If your app is bound to`127.0.0.1`(which is common in Vite apps), switch it to`0.0.0.0` to allow external access.
...
Templates and Compose Services: Use Traefik labels for routing configuration
...
Multi-TenancyUnderstand how Dokploy organizes resources using Organizations, Projects, Environments, and Services for team collaboration and resource isolation.

Dokploy CloudDeploy your apps to multiple servers remotely without worrying about the underlying infrastructure.

### On this page

Applications Domain Not Working?Logs and Monitoring Not Working After Changing Application Placement?Mounts Are Causing My Application Not to Run?Volumes in Docker Compose Not Working?I added a volume to my docker compose, but is not finding the volume?Using Files from Your RepositoryLogs Not Loading When Deploying to a Remote Server?Docker Compose Domain Not Working?Templates and Compose Services Returning 404Understanding the DifferenceSolutionGetting "Bad Gateway Error" When Accessing Your Application DomainCommon CausesCommon Solution for Modern JavaScript FrameworksExample Configuration for ViteFramework-Specific NotesDocker Compose Volume MountsUse of closed network when restarting TraefikCreating Configuration FilesFailed to initialize Docker SwarmMy Dokploy UI Instance is Not Accessible1. Insufficient Storage Space2. Container Race Condition During RestartDebugging Process1. Check Container Logs2. Common Database Connection Issue3. Traefik Configuration IssuesRecreate dokploy containersRecreate Redis ServiceRecreate Dokploy ServiceFinal Notes